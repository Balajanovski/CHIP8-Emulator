...
Drawer::Drawer(int w, int h, const std::string &title) : window_width(w), window_height(h) {
    if (SDL_Init(SDL_INIT_EVERYTHING) == -1) {
        std::runtime_error(std::strcat("error: Unable to initialise SDL ", SDL_GetError()));
    }
    ...
    if (!win) {
        std::runtime_error(std::strcat("error: Unable to create window ", SDL_GetError()));
    }

    ren = SDL_CreateRenderer(win, -1, SDL_RENDERER_PRESENTVSYNC | SDL_RENDERER_ACCELERATED);
    if (!ren) {
        std::runtime_error(std::strcat("error: Unable to create renderer ", SDL_GetError()));
    }
}
